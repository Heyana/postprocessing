...
window.addEventListener("load", () => load().then((assets) => {
	...
	renderer.debug.checkShaderErrors = true; // Always check shader errors during debugging
	renderer.shadowMap.type = VSMShadowMap;
	renderer.shadowMap.autoUpdate = false;
	renderer.shadowMap.needsUpdate = true;
	renderer.shadowMap.enabled = true;

	// Set explicit clear color for debugging
	renderer.autoClear = true;
	renderer.setClearColor(0x002200); // Dark green
	...
	// Add names to objects for better debugging
	actors.name = "Actors";
	...
	requestAnimationFrame(function render(timestamp) {
		...
		// First, try a direct render to see if the scene works normally
		if (timestamp < 1000) { // Only do this for the first second
			console.log("Direct scene render test");
			renderer.setRenderTarget(null);
			renderer.render(scene, camera);
		}

		// Then do the normal composer render
		composer.render();
