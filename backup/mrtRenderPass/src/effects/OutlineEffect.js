...
export class OutlineEffect extends Effect {
	 ...
	 * @param {DepthPass|SharedDepthPass} [depthPass] - An optional shared depth pass for optimized rendering.
	 */
	...
	update(renderer, inputBuffer, deltaTime, depthPass) {
		...
		if (this.forceUpdate || selection.size > 0) {
			...
			this.time += deltaTime;
			timeEndLog("OutlineEffect.update.preparation");

			log("OutlineEffect.update-depthpass存在么？", depthPass);
			// 使用共享的深度通道（如果提供）或自己的深度通道
			if (depthPass !== null && depthPass !== undefined) {
				timeLog("OutlineEffect.update.useSharedDepthPass");

				// 检查depthPass本身是否有效
				let isValidDepthPass = true;

				// 检查是否有texture属性
				if (!depthPass.hasOwnProperty('texture')) {
					console.warn('OutlineEffect: 提供的深度通道没有texture属性');
					isValidDepthPass = false;
				}

				// 检查texture是否为null或undefined
				if (depthPass.texture === undefined || depthPass.texture === null) {
					console.warn('OutlineEffect: 共享深度通道的纹理为null或undefined');
					isValidDepthPass = false;
				}

				if (isValidDepthPass) {
					log('OutlineEffect: 使用共享深度通道纹理');
					...
					if (depthMaterial) {
						...
						console.warn('OutlineEffect: maskPass 没有 overrideMaterial，回退到自己的深度通道');
					...
					console.warn('OutlineEffect: 共享深度通道无效，回退到自己的深度通道');
					// 回退到使用自己的深度通道
					selection.setVisible(false);
					this.depthPass.render(renderer);
					selection.setVisible(true);
