...
export class EffectPass extends Pass {
	 ...
	 * @param {SharedDepthPass} [sharedDepthPass] - 可选的共享深度通道。
	 */
	render(renderer, inputBuffer, outputBuffer, deltaTime, stencilTest, sharedDepthPass) {

		timeLog("EffectPass.render");

		log("EffectPass.render 开始执行，效果数量: " + this.effects.length, sharedDepthPass, "sharedDepthPass");

		// 验证共享深度通道
		let validSharedDepth = false;
		if (sharedDepthPass) {
			// 检查对象是否为null或undefined
			if (sharedDepthPass === null || sharedDepthPass === undefined) {
				log("EffectPass: 共享深度通道是null或undefined");
			}
			// 检查是否有texture属性
			else if (!sharedDepthPass.hasOwnProperty('texture')) {
				log("EffectPass: 共享深度通道没有texture属性");
			}
			// 检查texture是否有效
			else if (sharedDepthPass.texture === null || sharedDepthPass.texture === undefined) {
				log("EffectPass: 共享深度通道的纹理是null或undefined");
			}
			else {
				validSharedDepth = true;
				log("EffectPass: 共享深度通道有效，纹理ID: " + (sharedDepthPass.texture.id || "未知"));
			}
		}
		...
		for (const effect of effects) {
			const effectName = effect.constructor.name || "未知效果";
			timeLog(`EffectPass.${effectName}.update`);

			if (effect.update) {
				try {
					// 如果共享深度通道有效，则传递给效果的update方法
					// 否则传递null避免使用无效的共享深度通道
					if (validSharedDepth) {
						log(`EffectPass: 为${effectName}提供共享深度通道`);
						effect.update(renderer, inputBuffer, deltaTime, sharedDepthPass);
					} else {
						log(`EffectPass: ${effectName}使用自己的深度通道`);
						effect.update(renderer, inputBuffer, deltaTime, null);
					}
				} catch (e) {
					console.error(`EffectPass: ${effectName}更新过程中发生错误:`, e);
				}
			}

			timeEndLog(`EffectPass.${effectName}.update`);
		}
