...
void main() {
    // 首先读取输入纹理样本，检查是否为高亮标记物体
    vec4 inputSample = texture2D(inputTexture, vUv);
    float brightness = max(max(inputSample.r, inputSample.g), inputSample.b);
    
    // 如果检测到被标记为忽略AO的超亮物体，则保留原样
    ...
    for (int i = 0; i < samples; i++) {
        ...
        // 检查邻居像素是否是高亮物体
        vec4 neighborInputSample = texture2D(inputTexture, neighborUv);
        float neighborBrightness = max(max(neighborInputSample.r, neighborInputSample.g), neighborInputSample.b);
        if (neighborBrightness > IGNORE_BRIGHTNESS_THRESHOLD) {
            continue; // 跳过高亮像素
        }
