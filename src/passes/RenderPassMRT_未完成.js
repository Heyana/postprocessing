...
export class RenderPass extends Pass {
	...
	constructor(scene, camera, overrideMaterial = null, options = {}) {
		...
		/**
		 * A copy material for MRT optimization (copies MRT output to inputBuffer).
		 *
		 * @type {CopyMaterial}
		 * @private
		 */
		...
		/**
		 * A fullscreen quad for copying textures.
		 *
		 * @type {Mesh}
		 * @private
		 */
	...
	render(renderer, inputBuffer, outputBuffer, deltaTime, stencilTest, depthPass, renderOpts = {}, effectPassOpts) {
		...
		if (this.enableGBuffer && this.useMRTOptimization && this.mrtPatcher && this.enableObjectId) {
			...
			// ✅ 将MRT的第一个输出（lit color）复制到inputBuffer
			// 这样后续pass就能正确读取渲染结果了
			if (!this.renderToScreen && inputBuffer) {
				...
				if (!this.copyMaterial) {
					...
					// 创建全屏quad用于复制
					const copyGeometry = Pass.fullscreenGeometry;
					this.copyQuad = new Mesh(copyGeometry, this.copyMaterial);
				...
				// 设置输入纹理为MRT的第一个输出
				this.copyMaterial.inputBuffer = this.gBufferRenderTarget.textures[0];

				// 渲染到inputBuffer（使用全屏quad复制）
