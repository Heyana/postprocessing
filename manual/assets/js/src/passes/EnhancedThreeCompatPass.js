...
export class EnhancedThreeCompatPass extends Pass {
    ...
    setDepthTexture(depthTexture, depthPacking = BasicDepthPacking) {
        ...
        // 对于SSR特殊处理
        if (this.passType === "ssr" && this.threePass && this.threePass.ssrMaterial) {
        ...
        switch (this.passType) {
            case "ssr":
                ...
                if (this.threePass.beautyRenderTarget) {
                    // 确保beautyRenderTarget使用输入缓冲区的纹理
                    this.threePass.beautyRenderTarget = inputBuffer;

                    // 重要：确保SSRPass各种内部纹理都被正确设置
                    if (this.debug) {
                        console.log("SSR Pass缓冲区设置:", {
                            input: inputBuffer,
                            output: outputBuffer,
                            beauty: this.threePass.beautyRenderTarget,
                            depth: inputBuffer.depthTexture,
                            normal: this.threePass.normalRenderTarget
                        });
                    }

                    // 确保SSR Pass正确使用depthTexture
                    if (inputBuffer.depthTexture && this.threePass.ssrMaterial) {
    ...
    render(renderer, inputBuffer, outputBuffer, deltaTime, stencilTest, depthPass) {
        ...
        // 对于SSR特殊处理
        if (this.passType === "ssr") {
            // 确保使用正确的输出模式
            if (this.threePass.output !== undefined) {
                const originalOutput = this.threePass.output;
                // 先渲染一次获取缓冲区数据
                try {
                ...
                // 然后再渲染一次获取最终输出
                this.threePass.output = originalOutput;
            }
        }
