...
window.addEventListener("load", () => load().then((assets) => {
	...
	// Create a selective unreal bloom effect (使用 UnrealBloomPass 算法)
	const bloomEffect = new SelectiveUnrealBloomEffect(scene, camera, {
		blendFunction: BlendFunction.SCREEN,
		threshold: 0.3,
		smoothWidth: 0.2,
		strength: 3.0,  // 使用 strength 而不是 intensity
		radius: 0.4,
		nMips: 5
	});
	...
	titleElement.textContent = "Selective Unreal Bloom";
	...
	instructions.innerHTML = "点击场景中的对象切换泛光效果<br>使用 UnrealBloomPass 风格的多级模糊";
	...
	// Global explanation
	const folderInfo = mainPane.addFolder({ title: "Selective Unreal Bloom 说明" });
	const infoParams = {
		"info": "SelectiveUnrealBloomEffect 结合了选择性泛光和 UnrealBloomPass 风格的多级模糊效果，提供更好的视觉质量。点击物体可以添加或移除泛光效果。",
		"features": "• 只对选中的对象应用泛光\n• 使用多级下采样和上采样（类似 UnrealBloomPass）\n• 可以反转选择\n• 可以忽略背景\n• 可以调整多种参数"
	};
	folderInfo.addBinding(infoParams, "info", { label: "说明", multiline: true, readonly: true, rows: 3 });
	folderInfo.addBinding(infoParams, "features", { label: "功能", multiline: true, readonly: true, rows: 5 });
	...
	folderBloom.addBinding(bloomEffect, "radius", { min: 0, max: 1, step: 0.001 });
	folderBloom.addBinding(bloomEffect, "nMips", { min: 1, max: 8, step: 1, label: "MIP Levels" });
	folderBloom.addBinding(bloomEffect, "strength", { min: 0, max: 5, step: 0.1 });

	// Luminance settings
	const folderLuminance = mainPane.addFolder({ title: "Luminance Settings" });
	folderLuminance.addBinding(bloomEffect, "threshold", { min: 0, max: 1, step: 0.01 });
	folderLuminance.addBinding(bloomEffect, "smoothWidth", { min: 0, max: 0.1, step: 0.001 });
	...
	function updateTint() {
		...
		bloomEffect.bloomColor = new Color(colorParams.r, colorParams.g, colorParams.b);
